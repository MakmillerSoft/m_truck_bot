...
@router.message(VehicleCreationStates.waiting_for_photos, F.photo)
async def process_photos_input(message: Message, state: FSMContext):
    """Обробка завантаження фото авто (включаючи медіагрупи)"""
    try:
        ...
        if await process_media_group_photos(message, state):
            logger.info(f"📷 process_photos_input: фото оброблено як медіагрупа")
            return
        
        # Якщо не медіагрупа, обробляємо як одиночне фото
        logger.info(f"📷 process_photos_input: обробляємо як одиночне фото")
        
        # Отримуємо file_id фото
        ...
        # Показуємо кількість завантажених фото
        count = len(photos)
        text = f"""
🚛 <b>Створення картки авто</b>
...
"""
        ...
        # Відправляємо повідомлення з фото та текстом як підписом
        ...
        await state.update_data(last_photos_message_id=new_message.message_id)
        logger.info(f"📷 process_photos_input: створено нове повідомлення з фото {new_message.message_id}")
        
    except Exception as e:
        logger.error(f"❌ process_photos_input: помилка обробки фото: {e}", exc_info=True)
        await message.answer(
...
# ===== ОБРОБНИК ДОДАТКОВИХ ФОТО =====

@router.message(VehicleCreationStates.waiting_for_additional_photos, F.photo)
async def process_additional_photos_input(message: Message, state: FSMContext):
    """Обробка завантаження додаткових фото авто (включаючи медіагрупи)"""
    try:
        # Імпортуємо обробник медіагруп
        from .photo_group_processor import process_media_group_photos
        ...
        # Спочатку намагаємося обробити як медіагрупу
        if await process_media_group_photos(message, state):
            logger.info(f"📷 process_additional_photos_input: фото оброблено як медіагрупа")
            return
        
        # Якщо не медіагрупа, обробляємо як одиночне фото
        logger.info(f"📷 process_additional_photos_input: обробляємо як одиночне фото")
...
"""
        ...
        logger.info(f"📷 process_additional_photos_input: створено нове повідомлення з фото {new_message.message_id}")
        
    except Exception as e:
        logger.error(f"❌ process_additional_photos_input: помилка обробки фото: {e}", exc_info=True)
        await message.answer(
            "❌ Помилка завантаження фото",
...
<b>Додаткові фото:</b> Завантажте ще фото авто
